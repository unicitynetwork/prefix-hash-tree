...
describe('Sum-Certifying Tree', function() {
  it('should build a tree with numeric values and compute correct sums', async function() {
    const leaves: Map<bigint, SumLeaf> = new Map([
      [ 0b100000000n, { value: HexConverter.encode(await sha256('value-1')), numericValue: 100n } ],
      [ 0b100010000n, { value: HexConverter.encode(await sha256('value-2')), numericValue: 200n } ],
      [ 0b101000000n, { value: HexConverter.encode(await sha256('value-3')), numericValue: 300n } ]
    ]);
    
    const tree = new SumTree(new DataHasherFactory(NodeDataHasher), HashAlgorithm.SHA256, leaves, 8n);
    ...
    for (const [pathNum, leaf] of leaves) {
      ...
      assert.isTrue(await path.provesInclusionAt(pathNum), `Path ${pathNum.toString(2)} should be included`);
  ...
  it('should update sum when adding a leaf', async function() {
    ...
    tree.addLeaf(0b101000000n, {value: HexConverter.encode(await sha256('value-3')), numericValue: 300n});
    
    assert.equal(await tree.getRootSum(), 600n);
    
    const path = await tree.getProof(0b101000000n);
    assert.isTrue(await path.provesInclusionAt(0b101000000n));
    assert.equal(path.getLeafValue(), HexConverter.encode(await sha256('value-3')));
  ...
  it('should handle complex tree structures with mixture of left and right children', async function() {
    const leaves: Map<bigint, SumLeaf> = new Map([
      [ 0b1000n, { value: HexConverter.encode(await sha256('left-1')), numericValue: 10n } ],
      [ 0b1001n, { value: HexConverter.encode(await sha256('left-2')), numericValue: 20n } ],
      [ 0b1010n, { value: HexConverter.encode(await sha256('right-1')), numericValue: 30n } ],
      [ 0b1011n, { value: HexConverter.encode(await sha256('right-2')), numericValue: 40n } ]
    ]);
    
    const tree = new SumTree(new DataHasherFactory(NodeDataHasher), HashAlgorithm.SHA256, leaves, 3n);
    ...
    assert.equal(HexConverter.encode(path1.getRootHash()!), 'b61201658733cbd385e40c262b75dd583499242744e915ea04bd358b33362fd8');
    assert.equal(HexConverter.encode(path2.getRootHash()!), 'b61201658733cbd385e40c262b75dd583499242744e915ea04bd358b33362fd8');
    assert.equal(HexConverter.encode(path3.getRootHash()!), 'b61201658733cbd385e40c262b75dd583499242744e915ea04bd358b33362fd8');
    assert.equal(HexConverter.encode(path4.getRootHash()!), 'b61201658733cbd385e40c262b75dd583499242744e915ea04bd358b33362fd8');

    assert.equal(path1.getLeafValue(), HexConverter.encode(await sha256('left-1')));
    assert.equal(path2.getLeafValue(), HexConverter.encode(await sha256('left-2')));
    assert.equal(path3.getLeafValue(), HexConverter.encode(await sha256('right-1')));
    assert.equal(path4.getLeafValue(), HexConverter.encode(await sha256('right-2')));
...
describe('SumPath Validation', function() {
  describe('allNumericValuesOnPathArePositiveOrZero', function() {
    ...
    beforeEach(async function() {
      const leaves: Map<bigint, SumLeaf> = new Map([
        [positivePath, { value: await sha256('positive-leaf'), numericValue: 100n }],
        [negativePath, { value: await sha256('negative-leaf'), numericValue: -50n }]
      ]);
      
      setSkipNegativeValueCheck(true);
      try {
        treeWithNegativeValue = new SumTree(new DataHasherFactory(NodeDataHasher), HashAlgorithm.SHA256, leaves, 1n);
