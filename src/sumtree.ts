...
export class SumTree extends AbstractTree<
  SumInternalNode,
  SumLeafNode,
  SumLeaf,
  SumLeg,
...
> {
  public constructor(
    ...
    leavesByPath: Map<bigint, SumLeaf>,
    pathLengthInBits: bigint | false | undefined = undefined,
  ) {
  ...
  override async createPathForEmptyTree(): Promise<SumPath> {
    return new SumPath(
      [
        {
          type: "sumRoot",
          rootHash: await this.getRootHash(),
          sum: 0n,
        } as SumPathItemRoot,
      ],
      this.hashOptions,
  ...
  override async createPathItemRoot(): Promise<SumPathItemRoot> {
    return {
      type: "sumRoot",
      rootHash: await this.root.getHash(),
      sum: await this.root.getSum(),
    };
  }
  ...
  override async createEmptyLeftBranchPathItem(
    node: SumInternalNode,
  ): Promise<SumPathItemEmptyBranch> {
    return {
      type: "sumEmptyBranch",
      direction: LEFT,
      siblingHash: await node.right!.getHash(),
      siblingSum: await node.right!.getSum(),
    };
  }

  override async createEmptyRightBranchPathItem(
    node: SumInternalNode,
  ): Promise<SumPathItemEmptyBranch> {
    return {
      type: "sumEmptyBranch",
      ...
      siblingSum: await node.left!.getSum(),
    };
  }
  ...
  override async createPathItemInternalNodeHashed(
    node: SumInternalNode,
  ): Promise<SumPathItemInternalNodeHashed> {
    return {
      type: "sumInternalNodeHashed",
      nodeHash: await node.getHash(),
      sum: await node.getSum(),
    };
  }

  override async createPathItemLeafNode(
    leaf: SumLeafNode,
  ): Promise<SumPathItemLeaf> {
    return {
      type: "sumLeaf",
      value: leaf.getValue(),
      numericValue: await leaf.getSum(),
    };
  }

  override async addSiblingDataToLeft(
  ...
  override async addSiblingDataToRight(
    untypedPathItem: PathItem,
    node: SumInternalNode,
  ): Promise<void> {
    const pathItem = untypedPathItem as SumPathItemInternalNode;
  ...
  override createLeg(
    ...
    child: SumLeafNode | SumInternalNode,
  ): SumLeg {
    return new SumLeg(
      this.hashOptions,
      remainingPath,
...
export class SumLeafNode extends AbstractLeafNode<
  SumLeafNode,
  SumInternalNode,
  SumLeg
> {
...
export class SumInternalNode extends AbstractInternalNode<
  SumLeafNode,
  SumInternalNode,
  SumLeg
> {
...
export class SumLeg extends AbstractLeg<SumLeafNode, SumInternalNode, SumLeg> {
  ...
  public constructor(
    hashOptions: HashOptions,
    prefix: bigint,
    node: SumLeafNode | SumInternalNode,
...
> {
  public getLeafNumericValue(): bigint | undefined {
    ...
    if (
      !(leaf as SumPathItemLeaf).value ||
      (leaf as SumPathItemInternalNode).siblingHash ||
      (leaf as SumPathItemInternalNode).prefix
    ) {
      return undefined;
    }
  ...
  override createVerificationContext(
    hashOptions: HashOptions,
  ): VerificationContext {
    ...
    return {
      async beginCalculation(pathItem: PathItem): Promise<void> {
        if ("type" in pathItem && pathItem.type == "sumLeaf") {
          ...
          "type" in pathItem &&
          pathItem.type == "sumInternalNodeHashed"
        ) {
      ...
      async pathItemProcessed(pathItemAsSupertype: PathItem): Promise<void> {
        if (
          !(
            "type" in pathItemAsSupertype &&
            pathItemAsSupertype.type == "sumInternalNode"
          )
        ) {
          throw new Error("Unsupported PathItem type");
        }
      ...
      switch (item.type) {
        ...
        case "sumInternalNode":
          itemPayloadEncodedElements = [
            ...
            CborEncoder.encodeOptional(
              encodeBigintOrUndefined(item.siblingSum),
              CborEncoder.encodeByteString,
            ),
          ];
          break;
        ...
        case "sumEmptyBranch":
          itemPayloadEncodedElements = [
            CborEncoder.encodeTextString(item.type),
            CborEncoder.encodeByteString(
              BigintConverter.encode(item.direction),
            ),
            CborEncoder.encodeByteString(item.siblingHash),
            CborEncoder.encodeByteString(
              BigintConverter.encode(item.siblingSum),
            ),
          ];
          break;
        case "sumLeaf":
          ...
          itemPayloadEncodedElements = [
            ...
            encodedValue,
            CborEncoder.encodeByteString(
              BigintConverter.encode(item.numericValue),
            ),
          ];
          break;
        default:
          ...
          throw new Error(
            `Unknown SumPathItem type encountered during CBOR encoding: ${(_exhaustiveCheck as any).type}`,
          );
      }
    ...
    const jsonItems: AnySumPathItemJson[] = this.path.map((item) => {
      ...
      switch (concreteItem.type) {
        ...
        default:
          ...
          throw new Error(
            `Unknown SumPathItem type encountered during toJSON serialization: ${(_exhaustiveCheck as any).type}`,
          );
      }
    ...
    for (const item of obj.items) {
      ...
      switch (item.type) {
        case "sumRoot":
          ...
          if (
            !(
              typeof rootNode.rootHash === "string" &&
              typeof rootNode.sum === "string"
            )
          )
            return false;
          break;
        case "sumInternalNode":
          ...
          if (
            ...
            return false;
          if (
            internalNode.siblingSum !== undefined &&
            typeof internalNode.siblingSum !== "string"
          )
            return false;
          break;
        case "sumInternalNodeHashed":
          ...
          if (
            !(
              typeof hashedNode.nodeHash === "string" &&
              typeof hashedNode.sum === "string"
            )
          )
            return false;
          break;
        case "sumEmptyBranch":
          ...
          if (
            !(
              ...
              typeof emptyBranch.siblingSum === "string"
            )
          )
            return false;
          break;
        case "sumLeaf":
          ...
          if (
            !(
              typeof leafNode.value === "string" &&
              typeof leafNode.numericValue === "string" &&
              typeof leafNode.valueType === "string"
            )
          )
            return false;
          break;
  ...
  public static fromJSON(data: unknown, hashOptions: HashOptions): SumPath {
    ...
    const reconstructedPathItems: SumPathItem[] = data.items.map(
      (jsonItem: AnySumPathItemJson) => {
        switch (jsonItem.type) {
          ...
          case "sumInternalNode":
            return {
              ...
              siblingHash: jsonItem.siblingHash
                ...
                : undefined,
              siblingSum: jsonItem.siblingSum
                ? BigInt(jsonItem.siblingSum)
                : undefined,
          ...
          case "sumLeaf":
            ...
            const leafJson = jsonItem as ISumPathItemLeafJson & {
              valueType?: "string" | "Uint8Array";
            }; // Simulate valueType
          ...
          default:
            ...
            throw new Error(
              `Invalid item type in SumPath JSON data during reconstruction: ${(exhaustiveCheck as any).type}`,
            );
        }
      },
    );
...
function encodeBigintOrUndefined(
